...
type PlayerPhysicsConfig struct {
	...
	AccelTimeGround int     // Frames to reach full speed on ground
	AccelTimeAir    int     // Frames to reach full speed in air
	DecelTimeGround int     // Frames to stop on ground when no input
	DecelTimeAir    int     // Frames to stop in air when no input
	AccelCurve      string  // Easing curve for acceleration
	DecelCurve      string  // Easing curve for deceleration
	CurvePower      float64 // Power for "power" curve variant
...
func DefaultConfig() Config {
	return Config{
		...
		PlayerPhysics: PlayerPhysicsConfig{
			...
			AirControl:      1.0, // Full control in air (no slowdown when jumping)
			...
			MinJumpHeight:      3.0,  // 50% minimum jump
